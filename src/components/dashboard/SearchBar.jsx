...
// ─── Static pages + KPIs ────────────────────────────────────────────────────
const PAGES = [
  { label: "Tableau de Bord",                  description: "Vue d'ensemble générale",                    to: "/",            icon: LayoutDashboard, keywords: ["dashboard","accueil","overview"] },
...
function runSearch(q) {
  ...
  const pages = PAGES.filter((p) =>
    matches([p.label, p.description, ...p.keywords], q)
  ).slice(0, CATEGORY_META.pages.max);
  if (pages.length) results.pages = pages.map((p) => ({
    ...p, type: "pages", subtitle: p.description,
  }));

  // KPIs
  const kpis = KPIS.filter((k) =>
    matches([k.label, k.description, ...k.keywords], q)
  ).slice(0, CATEGORY_META.kpis.max);
  if (kpis.length) results.kpis = kpis.map((k) => ({
    ...k, type: "kpis", subtitle: k.description,
  }));

  // Clients
  const foundClients = clients.filter((c) =>
    matches([c.nom, c.code, c.region, c.segment], q)
  ).slice(0, CATEGORY_META.clients.max);
  if (foundClients.length) results.clients = foundClients.map((c) => ({
    label: c.nom,
    subtitle: `${c.code} · ${c.region} · ${c.segment}`,
    to: "/acteurs",
    icon: User,
    type: "clients",
  }));
  ...
  // Articles
  const foundArticles = articles.filter((a) =>
    matches([a.designation, a.code, a.famille], q)
  ).slice(0, CATEGORY_META.articles.max);
  if (foundArticles.length) results.articles = foundArticles.map((a) => ({
    label: a.designation,
    subtitle: `${a.code} · ${a.famille}`,
    to: "/produits",
    icon: Package,
    type: "articles",
  }));
  ...
  // Écritures
  const foundEcritures = ecritures.filter((e) =>
    matches([e.libelle, e.journal, e.compte, e.numPiece], q)
  ).slice(0, CATEGORY_META.ecritures.max);
  if (foundEcritures.length) results.ecritures = foundEcritures.map((e) => ({
    label: e.libelle,
    subtitle: `${e.numPiece} · ${e.journal} · ${e.date}`,
    to: "/fiscalite",
    icon: FileText,
    type: "ecritures",
  }));
  ...
  // Fournisseurs
  const foundFournisseurs = fournisseurs.filter((f) =>
    matches([f.nom, f.famille], q)
  ).slice(0, CATEGORY_META.fournisseurs.max);
  if (foundFournisseurs.length) results.fournisseurs = foundFournisseurs.map((f) => ({
    label: f.nom,
    subtitle: `${f.famille} · ${f.nbCommandes} commandes`,
    to: "/acteurs",
    icon: Truck,
    type: "fournisseurs",
  }));
  ...
  // Représentants
  const foundReps = representants.filter((r) =>
    matches([r.nom, r.region], q)
  ).slice(0, CATEGORY_META.representants.max);
  if (foundReps.length) results.representants = foundReps.map((r) => ({
    label: r.nom,
    subtitle: `${r.region} · ${r.nbClients} clients`,
    to: "/acteurs",
    icon: Users,
    type: "representants",
  }));
...
export function SearchBar() {
  ...
  const [activeKey, setActiveKey] = useState(null); // "catKey:index"
  ...
  const flat = categories.flatMap((cat) =>
    grouped[cat].map((item, i) => ({ cat, i, item }))
  );
  ...
  function handleKeyDown(e) {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = Math.min(activeFlat + 1, flat.length - 1);
      setActiveKey(`${flat[next].cat}:${flat[next].i}`);
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      const prev = Math.max(activeFlat - 1, 0);
      setActiveKey(`${flat[prev].cat}:${flat[prev].i}`);
    }
    if (e.key === "Enter") {
      e.preventDefault();
      const active = flat[activeFlat];
      if (active) go(active.item.to);
    }
  }
  ...
  return (
    <div className="relative flex-1 max-w-xl mx-3">
      {/* Input */}
      <div
        className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl border transition-all duration-200
          ${showDropdown && hasResults
            ? "border-primary/60 shadow-md shadow-primary/20 bg-surface rounded-b-none border-b-border/30"
      ...
      >
        ...
        {query && (
          <button
            ...
            className="text-text-dim hover:text-foreground transition-colors flex-shrink-0"
          >
            <X size={13} />
          </button>
        )}
      ...
      {showDropdown && (
        <div
          ref={dropdownRef}
          className="absolute left-0 right-0 top-full z-50 rounded-b-xl rounded-t-none border border-t-0 border-border bg-popover shadow-2xl shadow-black/30 overflow-hidden"
        >
          {hasResults ? (
            <>
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors
                              ${isActive ? `${meta.bg}` : "bg-surface-hover"}`}>
                            ...
                            {/* Arrow */}
                            {isActive && (
                              <ArrowRight size={13} className={`flex-shrink-0 ${meta.color}`} />
                            )}
                          </button>
                        );
                      })}
              ...
              <div className="border-t border-border/60 px-3 py-2 flex items-center justify-between bg-surface/50">
                <div className="flex items-center gap-3 text-[10px] text-text-dim">
                  <span className="flex items-center gap-1">
                    <kbd className="bg-surface-hover border border-border rounded px-1 font-mono">↑↓</kbd> naviguer
                  </span>
                  <span className="flex items-center gap-1">
                    <kbd className="bg-surface-hover border border-border rounded px-1 font-mono">↵</kbd> ouvrir
                  </span>
                  <span className="flex items-center gap-1">
                    <kbd className="bg-surface-hover border border-border rounded px-1 font-mono">Esc</kbd> fermer
                  </span>
                </div>
            ...
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-[11px] text-text-dim mt-1">
                Essayez : un nom de client, article, journal, page…
              </p>
            </div>
          )}
        </div>
